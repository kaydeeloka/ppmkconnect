...
const EditEventModal: React.FC<EditEventModalProps> = ({ isOpen, onClose, onEventUpdated, event }) => {
  ...
  useEffect(() => {
    if (event) {
      setFormData({
        ...
        participant_amount: event.participant_amount,
        description: event.description,
        inquiry_phone: event.inquiry_phone || '',
        inquiry_ig: event.inquiry_ig || '',
        fees: event.fees,
        kakao_link: event.kakao_link || ''
      })
    }
  }, [event])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await EventsAPI.updateEvent(event.id, formattedData)
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h2 className="text-2xl font-bold text-ppmk-dark">Edit Event</h2>
          <button
            onClick={onClose}
