...
interface FooterProps {
  ...
  setRedirectToCommunity: (redirect: boolean) => void
  setRedirectToMeetup: (redirect: boolean) => void
}

const Footer: React.FC<FooterProps> = ({ currentPage, setCurrentPage, setShowAuthModal, setRedirectToCommunity, setRedirectToMeetup }) => {
  const { user } = useAuth()
  
  const quickLinks = [
    { name: 'Home', page: 'home' },
    { name: 'Community', page: 'community', protected: true },
    { name: 'Events', page: 'activities' },
    { name: 'Meetup', page: 'meetup', protected: true },
  ]

  const handleFooterNavClick = (link: typeof quickLinks[0]) => {
    if (link.protected && !user) {
      if (link.page === 'community') {
        setRedirectToCommunity(true)
      }
      if (link.page === 'meetup') {
        setRedirectToMeetup(true)
      }
