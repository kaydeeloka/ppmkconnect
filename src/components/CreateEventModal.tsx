...
import { X, Calendar, MapPin, Users, DollarSign, Phone, MessageCircle, Link, Building } from 'lucide-react'
...
const CreateEventModal: React.FC<CreateEventModalProps> = ({ isOpen, onClose, onEventCreated }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Format datetime to ISO string for consistent storage
      const formattedData = {
        ...formData,
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h2 className="text-2xl font-bold text-ppmk-dark">Create New Event</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div>
            ...
            <input
              ...
              onChange={handleInputChange}
              placeholder="e.g., PPMK Seoul Chapter, Korean Language Club"
              className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-ppmk-dark/30 focus:border-ppmk-dark"
            />
            <p className="text-xs text-gray-500 mt-1">
              This will be displayed as "Organised by [Organisation Name]"
            </p>
          </div>
